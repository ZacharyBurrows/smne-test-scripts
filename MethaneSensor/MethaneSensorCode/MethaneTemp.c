/*
 * methaneTemp - Itron Methane Tool //itron-dynament-tool - Itron Dynament Tool
 *
 * Used for testing Itron serial connection to Heath NDIR CH4 Methane Sensor v1 //Dynament Gas Sensor
 *
 * For questions/support: norman.mcentire@itron.com , tobias.cain@itron.com
 *
 * 2017 July - Tobias Cain altered for use with Heath NDIR CH4 Methane Sensor v1
 * 2016 April - Updates
 * 2016 March - Original
*/

//to run: toolchain/bin/armv7l-timesys-linux-uclibcgnueabi-gcc MethaneTest.c -o methaneTemp -lpthread
...
//
// The Heath methane sensor continually spews data,
// It is read and converted inside of this code. 
// The information for this device may be found in
// 2.2 100T MS v1 System Interfaces Block Diagram
	...
	while (1) {
		...
		if (len < 0) {
			...
			if (raw_flag) {
				...
				if(sync_flag){ //checks if previous 2 bytes where sync and start
					//printf("0x%02x ", buf[buf_size]);
					dataArray[current_byte_count] = buf[buf_size]; //collects data element
					current_byte_count++; //increments array and counts data elements
					valid_data_count++;
				...
				//printf("0x%02x \n", temp);
				if(tempV == 0x0f && buf[buf_size] == 0x0c){
					sync_flag = 1;
					valid_data_count = 0;//resets valid data count
					//printf("\nFLAG SET!\n");
				}
				else if(valid_data_count > 38)
					sync_flag = 0;
...
***/
	...
	if (!raw_flag) {
		...
		if ((cmd == CMD_READ_LIVE_DATA) || (cmd == CMD_READ_LIVE_DATA_SIMPLE)) {	
			LiveData *live_data = (LiveData *)&buf[3];
			...
			ConfigData *config_data = (ConfigData *)&buf[3];
			printf("Version: 0x%04x\n", config_data->Version);
...
/////////////////////////////////////CONVERSIONS AND PRINT FUNCTIONS//////////////////////////////////////////////////
	float u = 3.0/32768.0;
